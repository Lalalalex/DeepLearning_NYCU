...
class ResNet152(nn.Module):
    def __init__(self, out_features = 2):
        ...
        self.layer1 = nn.Sequential(
            ...
            self.generate_layer(256, 64, 256, 2)
        )
        self.layer2 = nn.Sequential(
            Bottleneck(256, 128, 512, 2),
            self.generate_layer(512, 128, 512, 7)
        )
        self.layer3 = nn.Sequential(
            Bottleneck(512, 256, 1024, 2),
            self.generate_layer(1024, 256, 1024, 35)
        )
        self.layer4 = nn.Sequential(
            Bottleneck(1024, 512, 2048, 2),
            self.generate_layer(2048, 512, 2048, 7)
        )
        self.avgpool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        self.fc = nn.Linear(in_features = 2048, out_features = out_features, bias = True)
    def generate_layer(self, in_channel, cross_channel, out_channel, num_channels):
        layers = []
        for i in range(num_channels):
            layers.append(Bottleneck(in_channel, cross_channel, out_channel))
