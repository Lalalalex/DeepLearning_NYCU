...
def get_train_transforms():
    return albumentations.Compose([
        ...
        albumentations.GaussianBlur(always_apply=False, blur_limit = (3, 5), p = 0.2),
        #albumentations.GaussNoise(var_limit = (10.0, 50.0), mean = 0, always_apply = False, p = 0.2),
...
model = resNet18.ResNet18().to(cfg.device)
...
for epoch in range(cfg.epoch):
    ...
    train_epoch(model, train_data_loader, cross_entropy, optimizer)
    tta_model = ttach.ClassificationTTAWrapper(model, ttach.aliases.d4_transform(), merge_mode='mean')
    best_accuracy, best_loss = valid_epoch(tta_model, valid_data_loader, cross_entropy, best_accuracy, best_loss)